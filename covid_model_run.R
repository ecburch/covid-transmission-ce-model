...
covid_model_run_fixed_params_variant_scenarios <- function(par_fit, variant_scenario, n_yearly_variants, vaccination_scenario, time_horizon) {
  ...
  hosp_plot_name <- paste0("weekly_hospitalisations_", vaccination_scenario)
  deaths_plot_name <- paste0("weekly_deaths_", vaccination_scenario)
  ...
  a3 <- ggplot(hosp_df_weekly_summaries, aes(x = dates , y = inc_hosp_weekly_means)) + 
    ...
    annotate("label", x = as.Date(min(deaths_date) - 850), y = max(hosp_data_weekly$Total), label = "2023\nmax.", size = 6, colour = "black") +
    geom_hline(yintercept = min(hosp_data_weekly$Total), colour = "red", linetype = "dashed", linewidth = 1) +
    annotate("label", x = as.Date(min(deaths_date) - 850), y = min(hosp_data_weekly$Total), label = "2023\nmin.", size = 6, colour = "black") +
    scale_x_continuous(expand = expansion(mult = c(0.03, 0.03)), labels = seq(2023, 2124, by = 10),
                       breaks = seq(min(dates), max(dates), length = 11)) +
  ...
  b3 <- ggplot(deaths_df_weekly_summaries, aes(x = dates , y = inc_deaths_weekly_means)) + 
    ...
    annotate("label", x = as.Date(min(deaths_date) - 850), y = max(deaths_data_weekly$Total[4:52]), label = "2023\nmax.", size = 6, colour = "black") +
    geom_hline(yintercept = min(deaths_data_weekly$Total), colour = "red", linetype = "dashed", linewidth = 1) +
    annotate("label", x = as.Date(min(deaths_date) - 850), y = min(deaths_data_weekly$Total), label = "2023\nmin.", size = 6, colour = "black") +
    scale_x_continuous(expand = expansion(mult = c(0.03, 0.03)), labels = seq(2023, 2124, by = 10),
                       breaks = seq(min(dates), max(dates), length = 11)) +
    scale_y_continuous(limits = c(0, NA)) +
    theme(axis.text = element_text(size = 16),
  ...
  # ggsave(path = "Results/", filename = paste0(as.character(Sys.Date()), " Plot I. Weekly hosp and deaths.jpeg"),
  #        plot = weekly_hosp_deaths_plot, width = 20, height = 9)
  
  # Printing weekly hosp and death plots for 2023 only
  # 
  # a3 <- ggplot(hosp_df_weekly_summaries, aes(x = dates , y = inc_hosp_weekly_means)) +
  ...
  #   annotate("label", x = as.Date(min(deaths_date) + 20), y = 1881, label = "2023", size = 6, colour = "black") +
  #   scale_x_continuous(breaks = seq(min(deaths_date), max(deaths_date), by = 30.41), expand = c(0, 0)) +
  ...
  #   annotate("label", x = as.Date(min(deaths_date) + 20), y = 846, label = "2023", size = 6, colour = "black") +
  #   geom_hline(yintercept = 651, colour = "red", linetype = "dashed", linewidth = 1) +
  #   theme(panel.grid.minor = element_blank(),
  ...
  # weekly_hosp_deaths_plot <- grid.arrange(a3, b3, nrow = 2)
  # 
  # ggsave(path = "Results/", filename = paste0(as.character(Sys.Date()), " Plot I. Weekly hosp and deaths in 2023.jpeg"),
  #        plot = weekly_hosp_deaths_plot, width = 20, height = 10)
  # 
